...
socket.on('saveposter',function(data){
//console.log(data);
  var value = {title: data.title, image:data.image, building: data.building, time:data.time, category: data.category};
  console.log(value);
  db.collection('poster').ensureIndex( {image: 1}, {unique: true, dropDups: true}, null_function);
  db.collection('poster').insert( value, null_function);
});
...
socket.on('click_poster', function(data) {
  console.log(data);
  
   
var files = fs.readdirSync('public/images/'+data);
var array=[];

//console.log(files[i].substr(files[i].length-4));
for(var i=0; i<files.length;i++)
{   if(files[i].substr(files[i].length-4)=='.jpg'||files[i].substr(files[i].length-4)=='.png')
     array.push("/images/"+data+"/"+files[i]);
}
console.log(array);
socket.emit("poster",array);
socket.broadcast.emit("poster",array);


});
socket.on("nowclick", function(data) {
var title,building,time,category;
console.log(data);
 //var myCursor = db.collection("poster").find({image:data});
db.collection("poster").find({image: data}).toArray(function(err, items) {
  items.forEach(function(k, i){
    console.log(k["building"]);
    // title=k["title"];
    // building=k["building"];
    // time=k["time"];
    // category=k["category"];
